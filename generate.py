        ...
        if file_path not in processed_files:
            try:
                ...
                try:
                    ...
                    print(f"Warning: Could not read staged content for {file_path}: {str(e)}")
                    staged_content = ""
...
""".format(file_path=file_path)
    ...
    try:
        ...
        raise Exception(f"Failed to connect to Ollama server while processing {file_path}: {str(e)}")
...
def generate_combined_commit_message(file_summaries: Dict[str, str], use_emoji: bool = False) -> str:
    """
    Generate a combined commit message based on all file summaries
    """
...
"""
    ...
    try:
        ...
        raise Exception(f"Failed to connect to Ollama server while generating combined commit message: {str(e)}")
...
class GitChangeHandler(FileSystemEventHandler):
    ...
    def process_changes(self):
        if self.processing:
            logging.debug("Already processing changes, skipping...")
            return
